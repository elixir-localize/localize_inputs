if Code.ensure_loaded?(Ecto.Changeset) do
  defmodule Localize.Inputs.Changeset do
    @moduledoc """
    ...
    Only compiled when `:ecto` is loaded.
        ...
        def changeset(product, attrs) do
          ...
          |> Localize.Inputs.Changeset.validate_number(:quantity, min: 1)
    ...
    alias Localize.Inputs.Validator

    @doc """
    Validates a `Decimal` / integer field with the rules from
    `Localize.Inputs.Validator.validate_number/2`.
    ...
    * The changeset, with one error added per failing check.
    ...
    ### Examples
        ...
        iex> changeset = Localize.Inputs.Changeset.validate_number(changeset, :quantity, min: 1, max: 10)
        iex> changeset.valid?
        true
